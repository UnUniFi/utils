...
export class FaucetComponent implements OnInit {
  ...
  appPostFaucetRequested(faucetRequest: FaucetRequest): void {
    ...
    this.cosmosSDK.sdk$.subscribe(
      (sdk) => rest.cosmos.bank.allBalances(sdk.rest, address)
    ...
    const subscription = combinedLatest$.subscribe(
      ...
      ([_, sdk]) => {
        ...
        if (this.balancesBeforeFaucetRequest$ !== undefined && this.balancesAfterFaucetRequest$ !== undefined) {
          ...
          const balancesBefore = balancesBeforeFaucetRequest.balances ? balancesBeforeFaucetRequest.balances : [];
          const balancesAfter = balancesAfterFaucetRequest.balances ? balancesAfterFaucetRequest.balances : [];
          const resultList: {
            amount: number;
            denom: string;
            ...
            const matchedBalancesBefore = balancesBefore.length > 0 ? balancesBefore.filter((balance) => balance.denom === coin.denom) : [];
            const matchedBalancesAfter = balancesAfter.length > 0 ? balancesAfter.filter((balance) => balance.denom === coin.denom) : [];
            const matchedAmountBefore = matchedBalancesBefore.length > 0 ? matchedBalancesBefore[0].amount : '0';
            const matchedAmountAfter = matchedBalancesAfter.length > 0 ? matchedBalancesAfter[0].amount : '0';
            const amountDiff = parseInt(matchedAmountAfter ? matchedAmountAfter : '0', 10)
