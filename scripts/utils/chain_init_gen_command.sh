...
if [ "$execute_command" = "init" ]; then
  ...
  if (( major_version > 0 || minor_version >= 47 )); then
    echo "This is version 0.47 or later."
    $DAEMON_NAME genesis add-genesis-account my_validator 1000000000000uguu,100000000000ubtc;
    $DAEMON_NAME genesis add-genesis-account user1 100000000000uguu,100000000000ubtc,50000000000uusdc;
    $DAEMON_NAME genesis add-genesis-account user2 100000000000uguu,100000000000ubtc,50000000000uusdc;
    $DAEMON_NAME genesis add-genesis-account user3 100000000000uguu,100000000000ubtc,50000000000uusdc;
    $DAEMON_NAME genesis add-genesis-account user4 100000000000uguu,100000000000ubtc,50000000000uusdc;
    $DAEMON_NAME genesis add-genesis-account faucet 50000000000000uguu,500000000000ubtc,50000000000ueth,50000000000000uusdc;
    $DAEMON_NAME genesis add-genesis-account $PRICEFEED 100000000000uguu,100000000000000ubtc,100000000000000uusdc;
    $DAEMON_NAME genesis gentx my_validator 1000000000uguu --chain-id $CHAIN_ID --keyring-backend test;
    $DAEMON_NAME genesis collect-gentxs;
  else
    echo "This is before version 0.47."
